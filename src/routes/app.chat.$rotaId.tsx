...
import { getRota, getPessoa, mensagens, formatHora, eu } from "@/data/mock";
...
export const Route = createFileRoute("/app/chat/$rotaId")({
  ...
  notFoundComponent: () => <div className="p-8 text-sm text-muted-foreground">Rota não encontrada.</div>,
  component: Chat,
});

function Chat() {
  const r = Route.useLoaderData();
  const msgs = mensagens.filter((m) => m.rotaId === r.id);
    ...
    <div className="mx-auto flex h-[calc(100vh-7rem)] w-full max-w-3xl flex-col px-4 py-4 lg:px-8 lg:py-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-[10px] text-muted-foreground">{r.inscritos.length + 1} pessoas</p>
        </div>
      </div>
          ...
          return (
            <div key={m.id} className={`flex items-end gap-2 ${meu ? "flex-row-reverse" : ""}`}>
              ...
              <div className={`max-w-[75%] rounded-2xl px-3 py-2 text-sm ${
                ...
                {!meu && <p className="text-[10px] font-medium opacity-70">{autor.nome.split(" ")[0]}</p>}
                <p>{m.texto}</p>
      ...
      >
        ...
        />
        <button type="submit" className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-primary text-primary-foreground">
          <Send size={14} />
        </button>
      </form>
    </div>
